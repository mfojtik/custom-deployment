...
import (
	...
	core "k8s.io/client-go/1.5/kubernetes/typed/core/v1"
	clientset "k8s.io/client-go/1.5/kubernetes/typed/extensions/v1beta1"
	...
	intstrutil "k8s.io/client-go/1.5/pkg/util/intstr"
...
func GetAllReplicaSets(deployment *extensions.Deployment, c clientset.ExtensionsInterface, coreClient core.CoreInterface) ([]*extensions.ReplicaSet, []*extensions.ReplicaSet, *extensions.ReplicaSet, error) {
	rsList, err := listReplicaSets(deployment, c)
	if err != nil {
		return nil, nil, nil, err
	}
	podList, err := listPods(deployment, coreClient)
	if err != nil {
		return nil, nil, nil, err
	}
...
// Note that the first set of old replica sets doesn't include the ones with no pods, and the second set of old replica sets include all old replica sets.
func GetOldReplicaSets(deployment *extensions.Deployment, c clientset.ExtensionsInterface, coreClient core.CoreInterface) ([]*extensions.ReplicaSet, []*extensions.ReplicaSet, error) {
	rsList, err := listReplicaSets(deployment, c)
	if err != nil {
		return nil, nil, err
	}
...
func GetNewReplicaSet(deployment *extensions.Deployment, c clientset.ExtensionsInterface) (*extensions.ReplicaSet, error) {
	rsList, err := listReplicaSets(deployment, c)
	if err != nil {
		return nil, err
	}
...
func listReplicaSets(deployment *extensions.Deployment, c clientset.ExtensionsInterface) ([]*extensions.ReplicaSet, error) {
	return ListReplicaSets(deployment,
		func(namespace string, options api.ListOptions) ([]*extensions.ReplicaSet, error) {
			...
			for i := range rsList.Items {
				ret = append(ret, conversion.ReplicaSetToInternal(&rsList.Items[i]))
			}
			return ret, err
		})
}
...
func listPods(deployment *extensions.Deployment, c core.CoreInterface) (*api.PodList, error) {
	return ListPods(deployment,
		func(namespace string, options api.ListOptions) (*api.PodList, error) {
			versionedPodList, err := c.Pods(namespace).List(options)
			if err != nil {
				return nil, err
			}
			podList := &api.PodList{}
			if err := v1.Convert_v1_PodList_To_api_PodList(versionedPodList, podList, nil); err != nil {
				return nil, err
			}
			return podList, nil
		})
...
func WaitForReplicaSetUpdated(c clientset.ExtensionsInterface, desiredGeneration int64, namespace, name string) error {
...
func WaitForPodsHashPopulated(c clientset.ExtensionsInterface, desiredGeneration int64, namespace, name string) error {
...
func LabelPodsWithHash(podList *api.PodList, rs *extensions.ReplicaSet, c core.CoreInterface, namespace, hash string) (bool, error) {
	allPodsLabeled := true
	for _, pod := range podList.Items {
